...
class AuthorizationTokenValidator
  ...
  #   AuthorizationTokenValidator.new(authorization_token: 'valid-auth-token', purpose: :authorization).call
  #
  ...
  #   AuthorizationTokenValidator.new(refresh_token: 'valid-authorization-token', purpose: :test).call
  def call
  ...
  #
  # @return [Hash] {User} data encoded from the authorization token.
  #
  # @raise [ActiveSupport::MessageVerifier::InvalidSignature] when authorization token is invalid.
  def data
